...
account INT(9) NULL, id BIGINT(20) NULL, sms_type TINYINT(1) NULL DEFAULT '2', \
...
foreign_id VARCHAR(255) NULL, KEY sms_type (sms_type), KEY account (account), KEY receiver (receiver))"
...
msgdata, time, smsc_id, service, account, s.id, sms_type, mclass, mwi, coding, \
compress, validity, deferred, dlr_mask, dlr_url, pid, alt_dcs, rpi, \
charset, boxc_id, binfo, meta_data FROM %S s LEFT JOIN sms_campaign c ON c.id = s.account \
WHERE (c.enable=1 AND c.active=1 AND c.approved=1 AND c.ready=2 AND c.finished=0) OR (c.id IS NULL) \
LIMIT 0,1"
